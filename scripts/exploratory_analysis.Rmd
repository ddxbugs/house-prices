...
ggplot(data=processed_data, mapping=aes(SalePrice)) + geom_histogram(binwidth=10000, fill="blue", color="black") + 
  ...
  labs(title="Histogram of Home Sale Price ($ USD)")
...
```{r}
cor.test(processed_data$OverallCond, processed_data$SalePrice)
cor.test(processed_data$OverallQual, processed_data$SalePrice)
```
