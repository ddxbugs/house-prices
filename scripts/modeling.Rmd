...
coef(cv.out, s=lasso_best_lambda)
...
coef(cv.out, s=ridge_best_lambda)
...
```{r}
# Create dummy variable for categorical predictors
dummy_data <- model.matrix(SalePrice~., data=transformed_data)
...
plot(X1stFlrSF, SalePrice, data=transformed_data, main = "Bagging with Linear Regression")
...
```{r}
# Fit Random Forest
rf_model <- randomForest(SalePrice~., data = transformed_data, ntree = 5)
# Print model summary
print(rf_model)
