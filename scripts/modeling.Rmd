...
if (!require(caret)) install.packages("gridExtra")
if (!require(caret)) install.packages("scales")
library(caret)
library(glmnet)
library(olsrr)
library(car)
...
lasso.mod <- glmnet(transformed_data[,-SalePrice], log(SalePrice), alpha=1, lambda=grid)
cv.out <- cv.glmnet(transformed_data[,-SalePrice], log(SalePrice), alpha=1)
plot(cv.out)
best_lambda <- cv.ouit$lamdba.1se
...
# Create a formula
formula <- log(SalePrice)~X1stFlrSF

# Perform Bagging
bagging_model <- bagging(formula, data = transformed_data, nbagg = 50)

# Predict and plot
preds <- predict(bagging_model, transformed_data)
plot(X1stFlrSF, log(SalePrice), main = "Bagging with Linear Regression")
lines(X1stFlrSF, preds, col = "red", lwd = 2)
...
#   ggplot(transformed_data, aes_string(x = predictor, y = "SalePrice")) +
#     geom_point() +
#     geom_line(aes_string(y = "Predictions"), color = "blue") +
#     labs(title = paste(predictor, "vs SalePrice"),
#          x = predictor,
#          y = "SalePrice") +
#     theme_minimal()
# })
